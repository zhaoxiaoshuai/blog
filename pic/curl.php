...
class DownLoad {
     ...
     * @param  Int    $timeout           超时秒数 默认30秒
     * @param  String $logfile           日志文件路径
     */
    public function __construct($download_config = array(), $max_process_num=10, $timeout=30, $logfile=''){
    ...
    public function process($download_config){
        ...
        foreach($download_config as $k=>$config){
            $ch[$k] = curl_init();
            $path = pathinfo($config[1], PATHINFO_DIRNAME);
            if(!is_dir($path)){
    ...
    public function setDownload(Array $arr = array())
    {
        if (count($arr)>0)
            $this -> download_config = $arr;
        return $this;
    }
}